...
/**
 * Singleton catalog holding all available lending products, defined via the DSL.
 */
object ProductCatalog {
    ...
    private val catalog: Map<String, LendingProduct> = listOf(
        PERSONAL_INSTALLMENT_LOAN,
        REVOLVING_LINE_OF_CREDIT,
    ).associateBy { it.id }

    fun findById(id: String): LendingProduct? = catalog[id]

    fun all(): List<LendingProduct> = catalog.values.toList()
}
